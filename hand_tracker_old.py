...
class HandTracker:
    ...
    def find_position(self, img, results, hand_no=0, draw=True, smooth=True):
        ...
        if results.multi_hand_landmarks and len(results.multi_hand_landmarks) > hand_no:
            ...
            # Extract landmarks - keep normalized coordinates for proper screen mapping
            positions = []
            for id, lm in enumerate(hand.landmark):
                # Store normalized coordinates (0-1) for accurate transformation
                cx_norm, cy_norm = lm.x, lm.y
                # Convert to pixel coordinates for drawing only
                cx_pixel, cy_pixel = int(lm.x * w), int(lm.y * h)
                cz = lm.z  # Depth information

                # Store normalized coordinates for accurate screen mapping
                positions.append([id, cx_norm, cy_norm, cz])

                if draw:
                    # Color-code different finger parts
                    color = self._get_landmark_color(id)
                    cv2.circle(img, (cx_pixel, cy_pixel), 5, color, cv2.FILLED)
    ...
    def _calculate_hand_size(self, lm_list):
        """Calculate hand size based on wrist to middle finger tip distance (normalized coordinates)"""
        if len(lm_list) < 21:
            return 0
    ...
    def _calculate_hand_center(self, lm_list):
        """Calculate center point of hand (normalized coordinates)"""
        if not lm_list:
            return (0, 0)
    ...
    def _calculate_bounding_box(self, lm_list):
        """Calculate bounding box of hand (normalized coordinates)"""
        if not lm_list:
            return None
    ...
    def get_gesture(self, lm_list, hand_info=None, smooth=True):
        ...
        # Calculate dynamic thresholds based on hand size for normalized coordinates
        hand_size = hand_info.get("hand_size", 0.1) if hand_info else 0.1
        avg_hand_size = (
            sum(self.hand_size_history) / len(self.hand_size_history)
            if self.hand_size_history
            else hand_size
        )
        # Normalized pinch threshold (typical hand size is ~0.15 in normalized space)
        pinch_threshold = max(0.02, avg_hand_size * self.pinch_threshold_ratio)
